...
| `image ... not found` / `429` при pull | Хиккап Docker Hub или **лимит анонимных пулов** (100/6ч на IP). Повторить `... pull <сервис>`; при повторе — `docker login` (лимит ↑ до 200/6ч), затем `up -d`. Образы реальны — тег менять не нужно |
