...
| `image ... not found` / `429` при pull (падает то один образ, то другой) | **Лимит анонимных пулов** Docker Hub (100/6ч): `up` тянет параллельно и упирается в лимит. Скачайте последовательно: `bash pull-images.sh` (по одному, с ретраями), при повторе — `docker login` (лимит ↑ до 200/6ч), затем `up -d`. Образы реальны — тег менять не нужно |
| `no matching manifest for linux/arm64` | Образ только под amd64 (напр. `dvga`), а хост ARM/Apple Silicon. Запуск: `docker compose ... up -d` с `DOCKER_DEFAULT_PLATFORM=linux/amd64` (через эмуляцию), либо не поднимать эту мишень |
