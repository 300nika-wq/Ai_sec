...
> **Если `up` падает с `image ... not found` / `429`** — это лимит анонимных пулов
> Docker Hub (compose тянет образы параллельно и упирается в лимит). Скачайте их
> сначала **последовательно с ретраями**:
> ```bash
> bash pull-images.sh          # тянет по одному с повторами; при лимите — docker login и снова
> ```
> Затем обычный запуск ниже.
