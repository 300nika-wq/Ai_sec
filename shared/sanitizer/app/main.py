...
# Внешний коммерческий провайдер (пример: Anthropic-совместимый эндпоинт).
# Тариф обязан быть с Zero-Data-Retention и opt-out обучения.
...
async def _call_external(prompt: str, system: str | None, temp: float) -> str:
    ...
    async with httpx.AsyncClient(timeout=120) as c:
        r = await c.post(f"{EXTERNAL_URL}/v1/messages", headers=headers, json=body)
        r.raise_for_status()
        data = r.json()
